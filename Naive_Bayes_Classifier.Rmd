...
top_20_words_overall <- word_counts_by_label %>%
    group_by(word) %>%
    summarise(total_n = sum(n)) %>%
    top_n(20, total_n) %>%
    pull(word)

top_words_by_label <- word_counts_by_label %>%
    filter(word %in% top_20_words_overall) %>%
    mutate(
        percent = case_when(
...
word_order <- top_words_by_label %>%
  group_by(word) %>%
  summarize(max_n = sum(n)) %>%
  arrange(max_n) %>%
  pull(word)

top_words_by_label$word <- factor(top_words_by_label$word, levels = word_order)

ggplot(
  data = top_words_by_label,
  ...
  geom_text(
    ...
    hjust = ifelse(top_words_by_label$label == "discrim", 1.2, -0.2),
    size = 3
  ) +
  ...
  scale_x_continuous(
    labels = abs,
    limits = c(-max(top_words_by_label$percent) * 1.2, max(top_words_by_label$percent) * 1.2),
    breaks = scales::pretty_breaks(n = 6)
  ) +

  labs(
    title = "Top 20 Overall Words: Frequency by Sentiment Category",
